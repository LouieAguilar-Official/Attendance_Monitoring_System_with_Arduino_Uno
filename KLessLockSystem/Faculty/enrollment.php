...
if (isset($_POST['save'])) {
    ...
    if ($user_result->num_rows > 0) {
        while ($user_row = $user_result->fetch_assoc()) {
            ...
            if ($checkResult->num_rows > 0) {
                ...
                if ($updateStmt->execute()) {
                    $statusMsg = "<div class='alert alert-success'>Enrollment updated successfully for User ID: $user_id!</div>";
                } else {
                    $statusMsg = "<div class='alert alert-danger'>Error updating enrollment for User ID $user_id: " . $updateStmt->error . "</div>";
                }
                $updateStmt->close();
            } else {
                ...
                if ($insertStmt->execute()) {
                    ...
                    $statusMsg = "<div class='alert alert-danger'>Error creating enrollment for User ID $user_id: " . $insertStmt->error . "</div>";
                }
                $insertStmt->close();
            }
            $checkStmt->close();
        ...
        $statusMsg = "<div class='alert alert-danger'>No users found for the selected course, year, and section.</div>";
    }
    $stmt->close();
}
...
if (isset($_POST['delete'])) {
    ...
    if (!empty($enrollment_id)) {
        ...
        $statusMsg = "<div class='alert alert-danger'>Invalid enrollment ID.</div>";
    }
}
                                        ...
                                        <div class="table-responsive">
                                            ...
                                            <tbody>
                                                   <?php
                                                    ...
                                                    $enrollmentQuery = "
                                                        SELECT e.*, c.Course_name, y.Year_name, s.Section_name, 
                                                            l.Class_name, 
                                                            u.First_name, u.Last_name  -- Select First_name and Last_name
                                                        ...
                                                        JOIN user_table u ON e.User_ID = u.User_ID  -- Join with user_table
                                                    ...
                                                    if (mysqli_num_rows($result) > 0) {
                                                        while ($enrollmentRow = mysqli_fetch_assoc($result)) { ?>
                                                            ...
                                                            <td>
                                                                <?php 
                                                                    echo ucwords(htmlspecialchars($enrollmentRow['First_name'])) . " " . ucwords(htmlspecialchars($enrollmentRow['Last_name'])); 
                                                                ?>
                                                            </td>

                                                                <td><?php echo htmlspecialchars($enrollmentRow['Course_name']); ?></td>
                                                                <td><?php echo htmlspecialchars($enrollmentRow['Year_name']); ?></td>
                                                                <td><?php echo htmlspecialchars($enrollmentRow['Section_name']); ?></td>
                                                                <td><?php echo htmlspecialchars($enrollmentRow['Class_name']); ?></td>
                                                                <td>
